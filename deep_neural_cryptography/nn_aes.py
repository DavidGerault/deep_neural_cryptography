...
def warmup_model(model, batch_sizes, warmup_runs):
    compiled_model = torch.compile(model, mode="reduce-overhead", fullgraph=True, dynamic=True).eval()
    for batch_size in batch_sizes:
        warmup_data = torch.randint(0, 2, (batch_size, 128), device=DEVICE, dtype=DTYPE)
        with torch.inference_mode():
...
def run_benchmark_models(models, batch_sizes=[1, 10000], seed=42, num_repetitions=100, warmup_runs=50):
    ...
    for model, model_name in models:
        model = torch.compile(model.to(DEVICE), mode="max-autotune", fullgraph=True, dynamic=False).eval()
        for batch_size in batch_sizes:
